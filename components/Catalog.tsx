...
export default function Catalog({
  ...
  const tierInfo = catalog.tiers.find((item: any) => item.id === tier)

  const bundleCard = (bundle: any, type: string) => {
  ...
  return (
    <>
      ...
      <div className="sec-head" style={{ marginTop: 40, marginBottom: 18 }}>
        ...
        <p>Storage and features scale with your tier. Hosting is included in every price.</p>
      </div>
      <form className="tierbar" action="/join" method="get" aria-label="Storage tier">
        {catalog.tiers.map((item: any) => (
          <button
            type="submit"
            name="tier"
            value={`t${item.id}`}
            ...
            key={item.id}
            className={'tier' + (tier === item.id ? ' on' : '')}
            onClick={(event) => chooseTier(event, item.id)}
          >
            <span className="lbl">{item.label}</span>{item.storage}
          </button>
        ))}
      </form>

      <div className="pricing-note">
        <strong>One price. No hosting surcharge.</strong>
        <span>The amount shown includes the bundle, managed infrastructure, maintenance, and support.</span>
      </div>
      ...
      <div className="grid g3">
        {catalog.sovereign.map((bundle: any) => bundleCard(bundle, 'sovereign'))}
      </div>
      ...
      <div className="grid g3">
        {catalog.micro.map((bundle: any) => bundleCard(bundle, 'micro'))}
      </div>
